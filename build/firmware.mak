...
LDFLAGS-$(TARGET) := -mthumb -mcpu=cortex-m4 \
		  ...
		  -nostartfiles -Wl,-T,$(ROOT)/build/firmware.ld,--gc-sections,-Map,$(ROOT)/firmware/$(TARGET)/linker.map,-\(,-lc,-lm,--whole-archive,$(call spaceListToCommaList,$(LIBS:%=-l%)),--no-whole-archive,-\)
